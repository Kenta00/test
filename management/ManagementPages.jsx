...
import { FaBolt, FaCalendarDays, FaGear, FaGlobe, FaTriangleExclamation } from "react-icons/fa6";
  ...
  const diagnoseForm = async () => {
    ...
    try {
      const response = await fetch("/api/form-builder/generate", {
        ...
        body: JSON.stringify({ mode: "diagnose", prompt: "公開前診断", currentForm: toAuthoringForm(createFormFromActionItem(item)) }),
      });
      const result = await response.json().catch(() => null);
      if (!response.ok) throw new Error(result?.error || "AI診断に失敗しました。");
          ...
          <div className="space-y-5">
            ...
            <Card className="border-purple-200">
              <CardHeader className="flex flex-row items-center justify-between gap-3">
                <div><CardTitle className="flex items-center gap-2 text-lg"><FaBolt className="text-purple-600" />AIフォーム診断</CardTitle><p className="mt-1 text-sm text-slate-500">回答負担、曖昧さ、選択肢、個人情報、アクセシビリティを確認します。</p></div>
                <Button className="shrink-0 bg-purple-600 hover:bg-purple-700" disabled={isDiagnosing} onClick={diagnoseForm}>{isDiagnosing ? "診断中…" : diagnosis ? "再診断" : "AIで診断"}</Button>
              </CardHeader>
              <CardContent>
                {diagnosisError && <div className="rounded-md bg-red-50 p-3 text-sm text-red-700">{diagnosisError}</div>}
                {!diagnosis && !diagnosisError && <div className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500">診断を実行すると、公開前に改善できるポイントを優先度順に表示します。</div>}
                {diagnosis && (
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 rounded-lg bg-purple-50 p-4"><div className="text-3xl font-bold text-purple-700">{diagnosis.score}<span className="text-sm">/100</span></div><p className="text-sm leading-6 text-slate-700">{diagnosis.summary}</p></div>
                    {diagnosis.issues.length === 0 ? <div className="rounded-lg bg-green-50 p-4 text-sm text-green-700">大きな問題は見つかりませんでした。</div> : diagnosis.issues.map((issue, index) => {
                      const tone = issue.severity === "high" ? "border-red-200 bg-red-50 text-red-800" : issue.severity === "medium" ? "border-amber-200 bg-amber-50 text-amber-800" : "border-slate-200 bg-slate-50 text-slate-700";
                      return <div key={`${issue.title}-${index}`} className={`rounded-lg border p-4 ${tone}`}><div className="flex items-center gap-2 font-semibold"><FaTriangleExclamation />{issue.title}<span className="ml-auto text-xs">{issue.severity === "high" ? "優先度 高" : issue.severity === "medium" ? "優先度 中" : "優先度 低"}</span></div><p className="mt-2 text-sm leading-6">{issue.detail}</p>{issue.suggestion && <p className="mt-2 rounded-md bg-white/70 p-2 text-sm"><span className="font-medium">改善案:</span> {issue.suggestion}</p>}</div>;
                    })}
                  </div>
                )}
              </CardContent>
